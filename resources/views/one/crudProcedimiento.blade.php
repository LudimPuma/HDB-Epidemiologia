@extends('layout')
@section('title', 'Tablas | Procedimientos Invasivos')
@section('guide','Tablas / Procedimientos Invasivos')
@section('content')
<div class="row">
    <div class="col-12">
                    ...
                    <div class="mb-20">
                        @can('crud-create-procedimiento')
                            <button class="btn btn-success btn-insertar" style="padding: 1%;">
                                ...
                                <strong>Agregar Procedimiento</strong>
                            </button>
                        @endcan
                    </div>
                </div>
                        ...
                        <table id="dataTable" class="table mt-3 table-hover">
                            ...
                            <tbody class="text-muted">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @can('crud-edit-procedimiento')
                                            <button class="btn-editar text-muted" style="background: none; border: none; text-decoration: none;" data-id="{{ $procedimiento->cod_procedimiento_invasivo }}" data-nombre="{{ $procedimiento->nombre }}" data-estado="{{ $procedimiento->estado ? true : false }}" data-motivos="{{ $procedimiento->motivos_baja }}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalModificarLabel">Modificar Procedimiento</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="form-modificar" method="POST" action="{{ route('procedimiento.update', ['procedimiento' => ':id']) }}">
                    @csrf
                    @method('PUT')
...
<div class="modal fade" id="modalInsertar" tabindex="-1" aria-labelledby="modalInsertarLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="modalInsertarLabel">Insertar Procedimiento</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="form-insertar" method="POST" action="{{ route('procedimiento.store') }}">
                    @csrf
                    <div class="form-group">
                        <label for="nombre">Nombre</label>
                        <input type="text" id="nombre" name="nombre" class="form-control" required>
