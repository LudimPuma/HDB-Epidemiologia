@extends('layout')
@section('title', 'Tablas | Patologias')
@section('guide','Tablas / Patologias')
@section('content')
<div class="row">
    <div class="col-12">
                    ...
                    <div class="mb-20">
                        @can('crud-create-patologia')
                            <button class="btn btn-success btn-insertar" style="padding: 1%;">
                                ...
                                <strong>Agregar Patología</strong>
                            </button>
                        @endcan
                    </div>
                </div>
                        ...
                        <table id="dataTable" class="table mt-3 table-hover">
                            ...
                            <tbody class="text-muted">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @can('crud-edit-patologia')
                                            <button class="btn-editar text-muted" style="background: none; border: none; text-decoration: none;" data-id="{{ $patologia->cod_patologia }}" data-nombre="{{ $patologia->nombre }}" data-estado="{{ $patologia->estado ? true : false }}" data-motivos="{{ $patologia->motivos_baja }}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalModificarLabel">Modificar Patologia</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="form-modificar" method="POST" action="{{ route('patologia.update', ['patologia' => ':id']) }}" autocomplete="off">
                    @csrf
                    @method('PUT')
...
<div class="modal fade" id="modalInsertar" tabindex="-1" aria-labelledby="modalInsertarLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="modalInsertarLabel">Insertar Patología</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="form-insertar" method="POST" action="{{ route('patologia.store') }}" autocomplete="off">
                    @csrf
                    <div class="form-group">
                        <label for="nombre">Nombre</label>
                        <input type="text" id="nombre" name="nombre" class="form-control" required>
