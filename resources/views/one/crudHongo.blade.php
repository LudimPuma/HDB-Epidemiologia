@extends('layout')
@section('title', 'Tablas | Hongos')
@section('guide','Tablas / Hongos')
@section('content')
<div class="row">
    <div class="col-12">
                            ...
                            <tbody class="text-muted">
                                @foreach($hongos as $hongo)
                                    ...
                                    <td class="text-center">
                                        @can('crud-edit-hongo')
                                            <button class="btn-editar text-muted" style="background: none; border: none; text-decoration: none;" data-id="{{ $hongo->id }}" data-nombre="{{ $hongo->nombre }}" data-estado="{{ $hongo->estado ? true : false }}" data-motivos="{{ $hongo->motivos_baja }}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalModificarLabel">Modificar Hongo</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="form-modificar" method="POST" action="{{ route('hongo.update', ['hongo' => ':id']) }}">
                    @csrf
                    @method('PUT')
...
<div class="modal fade" id="modalInsertar" tabindex="-1" aria-labelledby="modalInsertarLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalInsertarLabel">Insertar Hongo</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="form-insertar" method="POST" action="{{ route('hongo.store') }}">
                    @csrf
                    <div class="form-group">
                        <label for="nombre">Nombre</label>
                        <input type="text" id="nombre" name="nombre" class="form-control" required>
